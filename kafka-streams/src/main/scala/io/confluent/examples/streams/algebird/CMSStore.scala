...
/**
  ...
  * on a percentage threshold, which means that 1. the set of heavy hitters will grow dynamically as
  * more unique items are being observed (counted) in the input and 2. this tracking of heavy
  * hitters adds to the (fixed) size of the traditional CMS data structure, giving the [[TopPctCMS]]
  * variant a larger and dynamic size.
  *
